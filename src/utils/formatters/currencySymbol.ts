/**
 * Currency Symbol Data and Lookup Utilities
 * Comprehensive list of world currencies with their symbols and ISO codes
 */
...
export const rawCurrencies: CurrencyInfo[] = [
  {
...
// ============================================================================
// Formatted Currency List (for dropdowns, selects, etc.)
// ============================================================================

const currencies: FormattedCurrency[] = rawCurrencies.map(({ abbreviation, currency, symbol }) => ({
  key: abbreviation,
  text: currency,
  id: abbreviation,
  value: abbreviation,
  symbol,
  description: `(${symbol} - ${abbreviation})`,
}));
...
currencies.sort((a, b) => collator.compare(a.text, b.text));
...
/**
 * Map of currency code to currency info for fast lookups
 */
const currencyByCode = new Map<string, CurrencyInfo>(
  rawCurrencies.map(c => [c.abbreviation.toUpperCase(), c]),
);

/**
 * Map of currency code to symbol for fast lookups
 */
const symbolByCode = new Map<string, string>(
  rawCurrencies.map(c => [c.abbreviation.toUpperCase(), c.symbol]),
);
 ...
 * @param code - Currency code (e.g., "USD", "EUR", "GBP")
 * @param fallback - Fallback symbol if not found (default: "$")
 * @returns Currency symbol
 */
export const getCurrencySymbol = (code: string | null | undefined, fallback = '$'): string => {
  if (!code) {
    return fallback;
  }
  return symbolByCode.get(code.toUpperCase()) ?? fallback;
};

/**
 * Get full currency info by ISO 4217 currency code
 * @param code - Currency code (e.g., "USD", "EUR", "GBP")
 * @returns Currency info or undefined if not found
 */
export const getCurrencyInfo = (code: string | null | undefined): CurrencyInfo | undefined => {
  if (!code) {
    return undefined;
  }
...
/**
 * Get currency name by ISO 4217 currency code
 * @param code - Currency code (e.g., "USD", "EUR", "GBP")
 * @param fallback - Fallback name if not found (default: "Unknown Currency")
...
export const getCurrencyName = (
  ...
  return currencyByCode.get(code.toUpperCase())?.currency ?? fallback;
};
...
export const getAllCurrencyCodes = (): string[] => {
  return Array.from(currencyByCode.keys());
};
 ...
 */
export const searchCurrencies = (query: string): FormattedCurrency[] => {
  if (!query) {
    return currencies;
  }
